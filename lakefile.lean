...
package «FutamuraAdelicBundle» where

require mathlib from git
  "https://github.com/leanprover-community/mathlib4" @ "v4.24.0"

-- Mathlib `v4.24.0` depends on auxiliary libraries specified as moving branches
-- in Mathlib’s own `lakefile.lean`. To keep `lake update` reproducible, pin the
-- ones we rely on to the revisions recorded in Mathlib’s `lake-manifest.json`
-- for `v4.24.0`.
