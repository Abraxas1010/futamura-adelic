...
/-- A tiny tautological “bound” lemma that keeps the relevant parameters in scope.

This is a placeholder for a future statement relating depth profiles to interpreter evaluation cost. -/
theorem depth_interpretation_bound
    ...
    (lens : LensID) :
    ∀ code input,
      ops.depth model.interp lens ≤ ops.depth model.interp lens ∧
        language.eval model.interp (split.pack code input) =
          language.eval model.interp (split.pack code input) := by
  intro code input
  exact ⟨le_rfl, rfl⟩
...
theorem specialization_finite_lens_activity
  ...
  classical
  refine
    (Finset.finite_toSet (s := ({LensID.omega, LensID.region, LensID.graph, LensID.clifford} : Finset LensID))).subset
      ?_
  intro lens _
  cases lens <;> simp
